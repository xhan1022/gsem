"""
retriver_multi_start_no_fill.py
多起点图遍历检索模块（消融版）。

与 retriver_multi_start.py 的唯一区别：
  - 若某个初始点这一路没有选中任何经验，不再将该初始点回填进结果。
  - 所有初始点都遵守这一规则；最终结果允许保持为空集。
"""
...
def _retrieve_from_start(
    ...
    print(f"\n{'=' * 60}")
    print(f"[Retrieve:SingleStartNoFill] Query  : {query[:120]}...")
    print(f"[Retrieve:SingleStartNoFill] Start  : {current_id}")
    print(f"[Retrieve:SingleStartNoFill] Pool   : {[nid for nid, _ in start_neighbors[:8]]}")
    print(f"{'=' * 60}")
    ...
    if not selected:
        print(f"[Start {start_id}] 未选中任何经验，保持空集（不回填起始点）")
...
def retrieve(case: dict, finder, max_steps: int = 60) -> list[dict]:
    ...
    print(f"\n{'#' * 60}")
    print(f"[Retrieve:MultiStartNoFill] 共 {len(start_ids)} 个起始点: {start_ids}")
    print(f"{'#' * 60}")

    for idx, start_id in enumerate(start_ids, 1):
        print(f"\n[Retrieve:MultiStartNoFill] 处理起始点 {idx}/{len(start_ids)}: {start_id}")
    ...
    print(f"\n[Retrieve:MultiStartNoFill] 最终合并 {len(merged)} 条经验: {[e['id'] for e in merged]}")
    return merged
